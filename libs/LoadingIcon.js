jQuery.sap.declare("zzuru.orders.orderSummaryZ_ORD_SUMMARY.libs.LoadingIcon");
(function ($) {
	$.loader_ext = {
		defaults: {
			autoCheck: 32,
		...
		create: function () {
			...
			this.loading = this.template($.loader.tmpl, {
				...
				Src: ops.imgUrl,
				Title: ops.title,
				ImgWidth: ops.imgWidth,
				ImgHeight: ops.imgHeight
			}).hide();
		...
		set_css: function () {
			...
			loader.css({
				'margin-top': (height - ops.size) / 3 + ops.position[0],
				'margin-left': (width - ops.size) / 2.5 + ops.position[1] - loader.find('span').outerWidth() / 2.5
			});
		},
		set_define: function () {
			var ops = this.options,
	...
	$.loader = {
		tmpl: '<div id="idLoadContainer" class="loading_wrp"><div class="loading ${Class}"><span id="idLoadingTxt">${Title}</span><br/><img width="${ImgWidth}" height=${ImgHeight} src="${Src}" /></div></div>',
		open: function (arg) {
